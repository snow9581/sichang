...
<head runat="server">
  ...
  <script type="text/javascript">
        ...
        $(function() {
            $('#dg').datagrid({
                title: "查看历史调查记录",
                url: "./viewHistorySurvey_DMKZ.ashx",
                fit: true,
                fitColumns: true,
                showFooters: true,
                collapsible: true,
                ...
                onLoadSuccess: function(data) {
                    if (data.rows.length > 0) {
                        ...
                        GMergeCells("dg", "INITDATE,MINERNAME,MINERCHECK,EXCELNAME0");
                    }
                },
                loader: SCLoader  
            });
        ...
        */
        ...
        function goExcel(val, row) {
           ...
           if((val!='#')&&(val!='')) return '<a href="downloadPic.aspx?picName=' + escape(row.EXCELNAME) + '&package=materialsubmit">下载</a>  '
        ...
        function goExcel0(val, row) {
           ...
           if((val!='#')&&(val!='')) return '<a href="downloadPic.aspx?picName=' + escape(row.EXCELNAME0) + '&package=materialsubmit">下载</a>  '


        }
        function goCheck(val, row) {

           if(val=='1')
           {              
              return '审核通过';
          } else if (val == '0') {
              if (row.MINEROPINION != "" && row.MINEROPINION != null)
                  row.MINEROPINION = row.MINEROPINION.replace(/\<br\/\>/g, "\n");
               return '<div style="cursor:pointer" onclick="alert(\'审核意见：'+row.MINEROPINION+'\')">拒绝 查看意见</div>';
            }else if(val=='')
            {
               return '等待审核';
            }
     ...
     <table id="dg">
        <thead>
            <tr>
                ...
                <th data-options="field:'MINERCHECK',formatter:goCheck"  width="50" >审核</th>
                <th hidden="true" field="MINEROPINION"  width="50">审核意见</th>
				<th data-options="align:'center',field:'EXCELNAME0',formatter:goExcel0"  width="50">汇总表报告</th>
				<th field="TEAMNAME"  width="50">小队</th>
				<%--<th field="WORDNAME"   width="50" >word下载</th>
				<th field="EXCELNAME"  width="50" >excel下载</th>--%>
				
				<th data-options="field:'EXCELNAME',formatter:goExcel"  width="50">小队报告</th>
			</tr>
        </thead>
    </table>
