<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ensure_wxd.aspx.cs" Inherits="ybManage_meterrepair_ensure_wxd" %>
    ...
    <script type="text/javascript">
        ...
        $(function () {
                ...
                $.messager.confirm("操作提示", "您确定要驳回吗？", function (data) {
                    if (data) {
                        $.messager.prompt("操作提示", "请输入您不同意的原因", function (data) {
                            var aj = $.ajax({
                                url: encodeURI(encodeURI("disagree.ashx?CONTENT=" + data + "&ID=" + $("#ID").val())),
                                type: 'post',
            ...
            $("#agree").click(function () {
                if (userlevel == "2") {
                    var aj = $.ajax({
                        url: encodeURI(encodeURI("agree.ashx?wxr=" + $("#wxr").combobox('getValue') + "&ID=" + $("#ID").val())),
                        type: 'post',
                        success: function (data) {
                            if (data == "1")
                                alert("操作成功！");
