...
<%@ Page Language="C#" AutoEventWireup="true" CodeFile="show_proofcheck.aspx.cs" Inherits="crud_tables_show_event" %>
...
///<summary> 
  ///模块编号：archives-08 电子档案管理子系统8号模块 校对审核记录文档表 table功能代码   
...
/// </summary>
        $(function() {
            $('#dg').datagrid({
                title: "校对审核记录文档",
                url: "../../crud_get/archives/get_proofcheck.ashx?pna=" + encodeURIComponent($('#HDpname').val()) + "&pnum=" + encodeURIComponent($('#HDpnumber').val()),
                fit: true,
                fitColumns: true,
                showFooters: true,
                collapsible: true,
                ...
                onExpandRow: function(index, row) {
                    ...
                    ddv.panel({
                        border: false,
                        cache: true,
                        href:'../../crud_form/archives/input_proofcheck.aspx?index=' + index,
                        onLoad: function() {
...
/// </summary>
        function saveItem(index) {
            ...
            //alert(row);
            var url = row.isNewRecord ? '../../crud_insert/archives/insert_proofcheck.ashx' : '../../crud_update/archives/update_proofcheck.ashx?ID='+row.ID;
           // alert(url);
            ...
            if (rows.length) {
                $.messager.confirm('提示', '你确定要删除已选记录吗?', function(r) {
                    ...
                    for(var i =rows.length-1;i>=0;i--)        
                        $.post('../../crud_destroy/archives/destroy_proofcheck.ashx', { ID: rows[i].ID });                   
...
///</summary>          
        function Save_Excel() {
            ...
            window.location="../../tools/toExcel.aspx?name=项目编号,项目名称,设计时间,档案号,设计类型,备注"+
            " &field=PID,PNAME,DESIGNRQ,FILENUMBER,DESIGNTYPE,BZ&location=T_proofcheck&condition="+escape(conditions);
     ...
     <table id="dg">
        <thead>
            <tr>
				...
				<th field="DESIGNTYPE"   width="50" sortable="true">设计类型</th>
				<th field="BZ"  width="50" >备注</th>
				<th data-options="field:'FILES',formatter:go"  width="50">校对审核文档</th>
