...
<%@ Page Language="C#" AutoEventWireup="true" CodeFile="show_PlanReply.aspx.cs" Inherits="crud_tables_show_event" %>
...
///<summary> 
  ///模块编号：archives-05 电子档案管理子系统5号模块 计划批复表 table功能代码   
...
/// </summary>
        $(function() {
            $('#dg').datagrid({
                title: "计划批复文档",
                url: "../../crud_get/archives/get_planReply.ashx?pna=" + encodeURIComponent($('#HDpname').val()) + "&pnum=" + encodeURIComponent($('#HDpnumber').val()) + "&ptype=" + encodeURIComponent($('#HDptype').val()),
                fit: true,
                fitColumns: true,
                showFooters: true,
                collapsible: true,
                ...
                onExpandRow: function(index, row) {
                    ...
                    ddv.panel({
                        border: false,
                        cache: true,
                        href:'../../crud_form/archives/input_planReply.aspx?index=' + index,
                        onLoad: function() {
...
/// </summary>
        function saveItem(index) {
            ...
            //alert(row);
            var url = row.isNewRecord ? '../../crud_insert/archives/insert_planReply.ashx' : '../../crud_update/archives/update_planReply.ashx?ID=' + row.ID;
           // alert(url);
            ...
            if (rows.length) {
                $.messager.confirm('提示', '你确定要删除已选记录吗?', function(r) {
                    ...
                    for(var i =rows.length-1;i>=0;i--)
                        $.post('../../crud_destroy/archives/destroy_planReply.ashx', { ID: rows[i].ID });                   
...
///</summary>          
        function Save_Excel() {
            ...
            window.location="../../tools/toExcel.aspx?name=项目编号,项目名称,下达时间,文件号,计划批复投资,文件类型,备注"+
            " &field=PID,PNAME,RELEASERQ,FILENUMBER,PLANMONEY,FILETYPE,BZ&location=T_planReply&condition="+escape(conditions);
     ...
     <table id="dg">
        <thead>
            <tr>
				...
				<th field="RELEASERQ" class="style1"sortable="true">下达时间</th>
				<th field="FILENUMBER" class="style2" sortable="true">文件号</th>
				<th field="PLANMONEY"  width="50" sortable="true">计划批复投资</th>
				<th field="FILETYPE"   width="50" sortable="true">文件类型</th>
				<th field="BZ"  width="50" >备注</th>
				<th data-options="field:'FILES',formatter:go"  width="50">批复文档</th>
