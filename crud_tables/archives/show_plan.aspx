...
<%@ Page Language="C#" AutoEventWireup="true" CodeFile="show_plan.aspx.cs" Inherits="crud_tables_show_event" %>
...
/// </summary>
        $(function() {
            $('#dg').datagrid({
                title: "方案文档",
                url: "../../crud_get/archives/get_plan.ashx?pna=" + encodeURIComponent($('#HDpname').val()) + "&pnum=" + encodeURIComponent($('#HDpnumber').val()),
                fit: true,
                fitColumns: true,
                showFooters: true,
                collapsible: true,
                ...
                onExpandRow: function(index, row) {
                    ...
                    ddv.panel({
                        border: false,
                        cache: true,
                        href:'../../crud_form/archives/input_plan.aspx?index=' + index,
                        onLoad: function() {
...
/// </summary>
        function saveItem(index) {
           ...
           //alert(row.ID);
            var url = row.isNewRecord ? '../../crud_insert/archives/insert_plan.ashx' : '../../crud_update/archives/update_plan.ashx?ID=' + row.ID;
            //alert(url);
            ...
            if (rows.length) {
                $.messager.confirm('提示', '你确定要删除已选记录吗?', function(r) {
                    ...
                    for(var i =rows.length-1;i>=0;i--)        
                        $.post('../../crud_destroy/archives/destroy_plan.ashx', { ID: rows[i].ID });                   
...
///</summary>          
        function Save_Excel() {
            ...
            window.location="../../tools/toExcel.aspx?name=项目编号,项目名称,方案编写时间,项目负责人,项目投资,主要工程量,关键字,投资类型,审核人,备注"+
            " &field=PID,PNAME,BXRQ,PLEADER,PMONEY,MAINWORK,KWORDS,INVESTTYPE,REVIEWER,BZ&location=T_plan&condition="+escape(conditions);
     ...
     <table id="dg">
        <thead>
            <tr>
				...
				<th field="BXRQ" class="style1"sortable="true">方案编写时间</th>
