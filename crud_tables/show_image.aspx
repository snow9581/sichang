<%@ Page Language="C#" AutoEventWireup="true" CodeFile="show_image.aspx.cs" Inherits="crud_tables_show_image" %>
        ...
        $(function () {
            $('#dg').datagrid({
                ...
                onExpandRow: function (index, row) {
                    if (row.INTRODUCTION != "" && row.INTRODUCTION != null)
                        row.INTRODUCTION = row.INTRODUCTION.replace(/\<br\/\>/g, "\n");
                    var ddv = $(this).datagrid('getRowDetail', index).find('div.ddv');
                    ddv.panel({
                        border: false,
                        cache: true,
                        href: '../crud_form/input_image.aspx?index=' + index,
                        onLoad: function () {
...
// <summary>  
        function saveItem(index) {
            ...
            //alert(row);
            var url = row.isNewRecord ? '../crud_insert/insert_image.ashx' : '../crud_update/update_image.ashx?ID=' + row.ID;
            //alert(url);
            ...
            if (rows.length) {
                $.messager.confirm('提示', '你确定要删除已选记录吗?', function (r) {
                    ...
                    for(var i =rows.length-1;i>=0;i--)         
                        $.post('../crud_destroy/destroy_image.ashx', { id: rows[i].ID });                   
        ...
        function go(val, row) {         
          if(row.IMGNAME!='#' && row.IMGNAME!=''&&row.IMGNAME!=undefined)  
            return '<a href="downloadPic.aspx?picName=' +escape(row.IMGNAME) + '&package=ftpupload" target="_blank">' +'下载' + '</a>  '
          else
            return ''
       }
///<summary>
...
//            var url = 'datagrid_to_excel.ashx'; //如果为asp注意修改后缀
...
//            return false;
            ...
            window.location="../tools/toExcel.aspx?name=矿名,队名,图纸简介,图纸类型,创建日期"+
            " &field=KM,DM,INTRODUCTION,IMGTYPE,CREATEDATE&location=T_IMAGE&condition="+escape(conditions);
  ...
  <table id="dg">
        <thead>
            ...
            <tr>
				...
				<th field="INTRODUCTION"   width="200" sortable="true">图纸简介</th>
				<th field="IMGTYPE"  width="100" sortable="true">图纸类型</th>
				<th field="IMGNAME" data-options="field:'PICTURE',formatter:go"   width="100" >图纸下载</th>
    ...
    <div id="toolbar">
        ...
        <span>图纸简介关键字:</span><input type="text" id="INTRODUCTION" value="" size="10" /> 
        ...
        <a href="../File/IMAGE.xls" class="easyui-linkbutton" iconCls='icon-download'">下载模板</a>
