<%@ Page Language="C#" AutoEventWireup="true" CodeFile="AssignmentWorkload_mine.aspx.cs" Inherits="plan_planRun_dtz_AssignmentWorkload" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        $(function () {
            $('#dg').datagrid({
                title: "工程量信息表",
                url: "AssignmentWorkload_getMine.ashx?pid=" + $('#PID').val(),
                fit: true,
                fitColumns: true,
                showFooters: true,
                collapsible: true,
                ...
                onExpandRow: function (index, row) {
                    ...
                    ddv.panel({
                        border: false,
                        cache: true,
                        href: 'AssignmentWorkload_inputMine.aspx?index=' + index + '&pid=' + pid + "&jurisdiction=" + $('#jurisdiction').val(),
                        onLoad: function () {
    ...
    function Save_Excel_aw() {
        ...
        var conditions = '';
        if ($('#PID').val() != "") {
            conditions += " and PID='" + $('#PID').val() + "' and Sender='规划室'";
        }
        else if (userlevel == "3") {
            conditions += " and Receiver = '" + $('#km').val() + "'";
        } 
        if ($('#Receiver').val() !="")
    ...
    function AssignmentWorkload(val, row) {
        return "<a href='#' onclick=\"parent.addTab('小队工程量','plan/planRun_dtz/AssignmentWorkload_team.aspx?id=" + row.PID + "&DM=" + row.RECEIVER + "')\">小队工程量</a>";
    }
</script>
</head>
<body>
 <table id="dg">
        <thead>    
            <tr>
                ...
                <th field="mineworkload" id="mw" width="80" data-options="formatter:AssignmentWorkload">小队工程量</th>
				...
				<th field="CONTENT" width="150">矿工作量</th>
				<th field="TIMERANGE"  width="150">当前时间节点</th>
                <th field="FEEDBACKINFORMATION"  width="150" data-options="styler:function(){return 'color:Orange;'}">矿反馈信息</th>
                <th field="CREATEDATE"  width="150">创建时间</th>
            </tr>
       </thead>
  </table>
  <input type="hidden" value="" id="km" runat="server"/>
  <input type="hidden" value="" id="PID" runat="server"/>
  <input type="hidden" value="" id="userLevel" runat="server"/>
  <input type="hidden" value="" id="jurisdiction" runat="server"/>
